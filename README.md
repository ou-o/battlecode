...
5. 玩家被自动跳进战斗页：屏幕中心有圆圈准星，识别到的 AprilTag 框上方显示名牌（昵称按阵营红/蓝着色）。「攻击」只打准星套住的那一个目标（须为可攻击单位），开火间隔最小 1s（客户端 + 服务端双重限制），服务端对被击单位扣 10 点。
   - 信息可见性（无全知视角）：面板常显自己 / 友方队友 / 己方基地血量；敌方玩家、掩体、敌方基地的信息只有镜头实际扫到标签才显示名牌，敌方血条还要被我方任一成员命中后才揭示（持续 10s，再命中刷新）。
6. 玩家 hp=0 → 阵亡，15s 倒计时结束后到本方基地前拍下基地标签（id 33 红 / 34 蓝）复位复活。
